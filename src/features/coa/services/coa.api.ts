...
import { accountsFormSchema, coaValidateSearch } from "#/features/coa/utils/schemas";
            ...
            .where(
                search
                    ? or(
                        ...
                        ilike(sql`cast(${ledgerAccounts.accountType} as text)`, `%${search}%`),
                    )
                    : undefined,
            )
...
export const deleteAccount = createServerFn({ method: "POST" }).middleware([authMiddleware])
